...
namespace networkScript.Expressions
{
	public class IdentifierList : Expression
	{
		...
		{
			throw new EvaluateException("Disallowed method call evaluate on visitable expression");
		}

		public new void visit(Action<Expression> visitor)
		{
			foreach (Identifier identifier in m_identifiers)
				visitor.Invoke(identifier);
		}
